{include file="backend/admin_header.tpl"}
                ...
                <div class="content">
                	...
                	<h3>{$LANG_USER_PROFILE} "{$user->login}"</h3>
					...
					<div class="admin_option">
						...
						<select name="status">
							<option value="1" {if $user->status == 1}selected{/if}>{$LANG_USER_STATUS_UNVERIFIED}</option>
							<option value="2" {if $user->status == 2}selected{/if}>{$LANG_USER_STATUS_ACTIVE}</option>
							<option value="3" {if $user->status == 3}selected{/if}>{$LANG_USER_STATUS_BLOCKED}</option>
						</select>
					</div>
