					<div class="admin_option">
						...
						<div id="img_wrapper">
							<div id="img_div_border" style="{if !isset($current_file)}display:none; {/if}width: {$attrs.width}px; height: {$attrs.height}px;">
									...
									<table width="{$attrs.width+4}px" height="{$attrs.height+4}px">
										<tr>
											<td valign="middle" align="center">
												<img id="img" src="{if isset($current_file)}{$upload_to}{$current_file}{/if}"/>
											</td>
										</tr>
									</table>
									<input type="hidden" name="{$upload_id}" id="{$upload_id}" value="{if isset($current_file)}{$current_file}{/if}">
								</div>
							</div>
