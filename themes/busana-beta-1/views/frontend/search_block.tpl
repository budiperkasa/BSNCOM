						...
						{/if}
                     	<div class="rounded">
			                ...
			                {/if}
                     		...
                     		<div class="search_filter_standart">
                     			<div>
                     				<a href="javascript: void(0);" id="what_match"><img style="padding-top: 8px;" src="{$public_path}images/icons/add.png"></a> <input type="text" class="what_where_search_input" name="what_search" id="what_search" value="{if $args.what_search}{$args.what_search}{else}{$LANG_SEARCH_WHAT} ({$LANG_SEARCH_WHAT_DESCR}){/if}" />
                     			</div>
                     			<div id="what_advanced_block" style="display: none;">
                     				<label><input type="radio" name="what_match" value="any" {if $args.what_match == 'any'}checked{/if} /> {$LANG_ANY_MATCH}</label>
	                     			<label><input type="radio" name="what_match" value="exact" {if $args.what_match == 'exact' || !$args[$field_mode] }checked{/if} /> {$LANG_EXACT_MATCH}</label>
	                     		</div>
                     		</div>
                     		{/if}
