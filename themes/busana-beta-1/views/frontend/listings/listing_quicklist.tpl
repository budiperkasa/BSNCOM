...
{assign var="listing_unique_id" value=$listing->getUniqueId()}
								...
								<div>
									<div class="remove_favourite" id="{$listing_id}"><a href="javascript: void(0);">{$LANG_REMOVE_FROM_QUICK_LIST}</a></div>
									<div id="listing_id-{$listing_unique_id}" class="listing_preview {if $listing->level->featured}featured{/if}">
										<div class="listing_head">
		                         			...
		                         			<div style="text-align: right; float: right">
												...
												{/if}
												{if $listing->level->videos_count && $listing->getAssignedVideos()}
												...
												{if $listing->type->locations_enabled && $listing->locations_count(true) && $listing->level->maps_enabled}
													<a href="#maps_canvas" id="open_iw_{$listing_unique_id}" title="{$LANG_MAP}"><img src="{$public_path}/images/buttons/map.png" /></a>&nbsp;
												{/if}
