...
ALTER TABLE `categories_by_type` ADD `selected_icons_serialized` text NOT NULL AFTER `seo_name`;
ALTER TABLE `categories_by_type` ADD `tree_path` varchar(255) NOT NULL AFTER `parent_category_id`;
CREATE INDEX `tree_path` ON `categories_by_type` (tree_path);
...
INSERT INTO `system_settings` SET `name`='W2D_VERSION', `value`='2.5.1' ON DUPLICATE KEY UPDATE `value`='2.5.1';