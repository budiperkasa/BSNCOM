...
class CI_DB_active_record extends CI_DB_driver {
	...
	function CI_DB_active_record($params)
	{
		parent::CI_DB_driver($params);

		$this->query_id = mt_rand();
	}
	...
	function select($select = '*', $escape = NULL)
	{
		...
		foreach ($select as $val)
		{
			...
			if ($val != '')
			{
				...
				events::callEvent('onQuerySelect', $val, $this->query_id);
		...
		foreach ((array)$from as $val)
		{
			if (strpos($val, ',') !== FALSE)
			{
				foreach (explode(',', $val) as $v)
				{
					...
					events::callEvent('onQueryFrom', $this->_protect_identifiers($v, TRUE, NULL, FALSE), $this->query_id);
			...
			else
			{
				...
				events::callEvent('onQueryFrom', $this->_protect_identifiers($val, TRUE, NULL, FALSE), $this->query_id);
	...
	{		
		...
		events::callEvent('onQueryJoin', $this->_protect_identifiers($table, TRUE, NULL, FALSE), $this->query_id);
	...
	{
		return $this->or_where($key, $value, 'AND', $escape);
	}
	...
	function _where($key, $value = NULL, $type = 'AND ', $escape = NULL)
	{
		...
		foreach ($key as $k => $v)
		{
			...
			events::callEvent('onQueryWhere', $k, $v, '=', $prefix.$k.$v, $this->query_id);
	 ...
	 * @return	object
	 */
	function where_in($key = NULL, $values = NULL, $escape = TRUE)
	{
	 ...
	 * @return	object
	 */
	function or_where_in($key = NULL, $values = NULL, $escape = TRUE)
	{
	 ...
	 * @return	object
	 */
	function where_not_in($key = NULL, $values = NULL, $escape = TRUE)
	{
	 ...
	 * @return	object
	 */
	function or_where_not_in($key = NULL, $values = NULL, $escape = TRUE)
	{
	...
	function _where_in($key = NULL, $values = NULL, $not = FALSE, $type = 'AND ', $escape = TRUE)
	{
		...
		events::callEvent('onQueryWhere', $key, implode(", ", $this->ar_wherein), '=', $where_in, $this->query_id);
	...
	function _like($field, $match = '', $type = 'AND ', $side = 'both', $not = '')
	{
		...
		foreach ($field as $k => $v)
		{
			...
			events::callEvent('onQueryLike', $k, $v, $like_statement, $this->query_id);
	...
	function set($key, $value = '', $escape = TRUE)
	{
		...
		foreach ($key as $k => $v)
		{
			if ($escape === FALSE)
			{
				...
				events::callEvent('onQuerySet', $this->_protect_identifiers($k), $this->query_id);
			}
			else
			{
	...
	{	
		...
		events::callEvent('onQueryInsert', $this->_protect_identifiers($table, TRUE, NULL, FALSE), $this->query_id);
		events::callEvent('onExecQuery', $this, $this->query_id, 'insert');
	...
	function update($table = '', $set = NULL, $where = NULL, $limit = NULL)
	{
		...
		events::callEvent('onQueryUpdate', $this->_protect_identifiers($table, TRUE, NULL, FALSE), $this->query_id);
		events::callEvent('onExecQuery', $this, $this->query_id, 'update');
	...
	function empty_table($table = '')
	{
		...
		events::callEvent('onQueryDelete', $table, $this->query_id);
	...
	{
		events::callEvent('onExecQuery', $this, $this->query_id, 'select');
	...
	function _reset_select()
	{
		...
		events::callEvent('onResetQuery', $this->query_id);
	}
	...
	*/
	...
	function on_duplicate_insert($table = '', $set = NULL )
	{
	    if ( ! is_null($set))
	    {
	        $this->set($set);
	    ...
	    events::callEvent('onExecQuery', $this, $this->query_id, 'insert_update');
	...
	/**
     * Modifying query functions starts from here
     *
     */

    public function changeSelectAlias($value, $alias)
    {
    	$a = explode(' as ', strtolower($value));
		if (count($a) == 1) {
			$a = explode(' ', strtolower($value));
		}
		$_field = trim($a[0]);
    	
    	if ($key = array_search($value, $this->ar_select))
    		$this->ar_select[$key] = $_field . ' AS ' . $alias;
    }
    
    public function changeSetKey($column, $tocolumn)
    {
    	if (array_key_exists($column, $this->ar_set)) {
    		$value = $this->ar_set[$column];
    		unset($this->ar_set[$column]);
    		$this->ar_set[$tocolumn] = $value;
    	}

    	/*if ($key = array_search($column, $this->ar_insert_mass_keys)) {
    		unset($this->ar_insert_mass_keys[$key]);
    		$this->ar_insert_mass_keys[$key] = $tocolumn;
    		ksort($this->ar_insert_mass_keys);
    	}*/
    }
    
    public function changeWhereClause($where_clause, $where_field, $to_where_field)
    {
    	$key = array_search($where_clause, $this->ar_where);
    	$this->ar_where[$key] = str_replace($where_field, $to_where_field, $where_clause);
    }
    
    public function changeLikeClause($like_clause, $like_field, $to_like_field)
    {
    	$key = array_search($like_clause, $this->ar_like);
    	$this->ar_like[$key] = str_replace($like_field, $to_like_field, $like_clause);
    }
}
