...
class CI_DB_result {
	...
	function result_array()
	{
		...
		while ($row = $this->_fetch_assoc())
		{
			foreach ($row AS $key=>$val) {
				if ($val == 'untranslated' || $val == null) {
					if (isset($row['original_' . $key])) {
						$row[$key] = $row['original_' . $key];
					}
				}
			}
	...
	function row($n = 0, $type = 'object')
	{
		if ( ! is_numeric($n))
		{
			...
			{
				foreach ($this->row_data[$n] AS $key=>$val) {
					if ($val == 'untranslated' || $val == null) {
						if (isset($this->row_data[$n]['original_' . $key])) {
							$this->row_data[$n][$key] = $this->row_data[$n]['original_' . $key];
						}
					}
				}
