...
class CI_Image_lib {
	...
	function image_process_gd($action = 'resize')
	{
		...
		// If the target width/height match the source or less than the source, AND if the new file name is not equal to the old file name
		...
		// -------------------------------------------------------------------------------
		// Png/Gif image transparency fix
		//
	 ...
	 * @param string $image
	 * @param array $destImageArray
	 * @param array $destImageSizeArray
	 */
	public function process_images($function, $image, $destImageArray, $destImageSizeArray, $destImageCrop = array())
    {
    	$CI = &get_instance();
    	$users_content_server_path = $CI->config->item('users_content_server_path');
    	$tmp_image = $users_content_server_path . 'tmp/' . basename($image);

    	foreach ($destImageArray as $key => $destImageItem) {
			copy($image, $tmp_image);
	        $sizeArray = explode('*', $destImageSizeArray[$key]);    

			if (isset($destImageCrop[$key]) && $destImageCrop[$key]) {
				$image_size = getimagesize($tmp_image);
				$config = array();
				$config['image_library'] = 'gd2';
				$config['source_image'] = $tmp_image;
				$config['maintain_ratio'] = FALSE;
				if ($image_size[0]/$image_size[1] > $sizeArray[0]/$sizeArray[1]) {
					$crop_width = ($image_size[1]*$sizeArray[0])/$sizeArray[1];
					$config['x_axis'] = ($image_size[0]-$crop_width)/2;
					$config['y_axis'] = 0;
					$config['width'] = $crop_width;
					$config['height'] = $image_size[1];
				} else {
					$crop_height = ($sizeArray[1]*$image_size[0])/$sizeArray[0];
			...
			$config['image_library'] = 'gd2';
			$config['source_image'] = $tmp_image;
			$config['new_image'] = $destImageItem;
			$config['create_thumb'] = TRUE;
			$config['thumb_marker'] = '';
			$config['maintain_ratio'] = TRUE;
			$config['width'] = $sizeArray[0];
			$config['height'] = $sizeArray[1];
