...
class CI_Loader {
	...
	{	
		...
		$this->_ci_view_path = BASEPATH.'views/';
	 ...
	 * @return	void
	 */	
	function model($model, $module = null, $name = '', $db_conn = FALSE)
	{
		if (is_array($model))
		{
		...
		if ( ! file_exists(MODULES_PATH . $module . '/models/' . $model . '_model' . EXT))
		{
		...
		require_once(MODULES_PATH . $module . '/models/' . $model . '_model' . EXT);
	 ...
	 * @return	void
	 */
	function view($register_triggers = true/*$view, $vars = array(), $return = FALSE*/)
	{
		$view = new view($register_triggers);
		return $view;
	...
	function helper($helpers = array())
	{
		...
		{		
			...
			$ext_helper = BASEPATH.'helpers/'.config_item('subclass_prefix').$helper.EXT;
			...
			elseif (file_exists(BASEPATH.'helpers/'.$helper.EXT))
			{ 
	...
	{	
		...
		{
			$subclass = BASEPATH.'libraries/'.$subdir.config_item('subclass_prefix').$class.EXT;
			...
			/*for ($i = 1; $i < 3; $i++)
			{
				...
				$filepath = BASEPATH.'libraries/'.$subdir.$class.EXT;
	...
	{	
		...
		if ($config === NULL)
		{
			...
			if (file_exists(BASEPATH.'config/'.strtolower($class).EXT))
			{
				include_once(BASEPATH.'config/'.strtolower($class).EXT);
			}			
			else
			{
				if (file_exists(BASEPATH.'config/'.ucfirst(strtolower($class)).EXT))
				{
					include_once(BASEPATH.'config/'.ucfirst(strtolower($class)).EXT);
				}			
			}
		}
	...
	/**
	 * Determines if module was loaded
	 *
	...
	function is_module_loaded($module_name)
	{
		...
		return array_key_exists($module_name, $modules_array);
	}
