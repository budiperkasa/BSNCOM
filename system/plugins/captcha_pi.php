...
|
*/
function create_captcha($CI)
{
	$data = array(
       	'word' => mt_rand(0, 9999),
       	'img_path' => $CI->config->item('users_content_server_path') . 'captchas/',
       	'img_url' => $CI->config->item('users_content_http_path') . 'captchas/',
       	'font_path' => BASEPATH . DIRECTORY_SEPARATOR . 'fonts' . DIRECTORY_SEPARATOR . 'ANTSYPAN.TTF',
       	'img_width' => '100',
       	'img_height' => '30',
       	'expiration' => '3600'
	);
   ...
   if ($word == '')
   {
	...
	$captcha = new captcha($word, $now, $img);
	return $captcha;
}

/**
 * captcha class
 * incapsulates generated data and outputs view
 *
 */
class captcha
{
	...
	public function __construct($word, $time, $img)
	{
		$this->word = $word;
		$this->time = $time;
		$this->image = $img;
		...
		$CI = &get_instance();
		// Set captcha word into session
		$CI->session->set_userdata(array('captcha_word' => $word));
	}
	...
	public function view()
	{
		$CI = &get_instance();
		$view = $CI->load->view();
		$view->assign('captcha', $this);
		return $view->fetch('backend/captcha.tpl');
	}
}
