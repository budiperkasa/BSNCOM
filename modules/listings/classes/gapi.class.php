...
class gapi
{
  ...
  private function fopenRequest($url, $get_variables=null, $post_variables=null, $headers=null)
  {
	...
	curl_setopt($curl_handle, CURLOPT_URL, $url . $get_variables);
	curl_setopt($curl_handle, CURLOPT_CONNECTTIMEOUT, 3);
	curl_setopt($curl_handle, CURLOPT_RETURNTRANSFER, 1);
	if(is_array($post_variables)) {
		curl_setopt($curl_handle, CURLOPT_POST, TRUE);
		curl_setopt($curl_handle, CURLOPT_POSTFIELDS, $http_options['content']);
	}
	curl_setopt($curl_handle, CURLOPT_HTTPHEADER, explode('\r\n', $http_options['header']));
	$response = curl_exec($curl_handle);
	curl_close($curl_handle);
    
    return array('body'=>$response!==false?$response:'Request failed, curl_exec provides no further information','code'=>$response!==false?'200':'400');
  }
